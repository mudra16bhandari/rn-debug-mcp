...
async function getAndroidLogs(limit: number, deviceId?: string, filter?: string): Promise<string> {
    ...
    try {
        ...
        if (filter && (!stdout || stdout.trim() === '--------- beginning of main' || stdout.length < 50)) {
            const { stdout: allLogs } = await execAsync(`adb ${deviceArgs} logcat -d -t ${searchLimit}`, execOptions);
...
async function getIosLogs(limit: number, deviceId: string = 'booted', filter?: string): Promise<string> {
    try {
