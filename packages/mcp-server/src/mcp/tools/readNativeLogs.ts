...
export const readNativeLogsSchema = z.object({
  ...
  filter: z
    .string()
    .optional()
    .describe('Optional filter string (e.g., a tag for Android or predicate for iOS)'),
});

export async function readNativeLogs(input: z.infer<typeof readNativeLogsSchema>): Promise<string> {
